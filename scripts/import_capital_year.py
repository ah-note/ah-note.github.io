...
def install(source, destination, company, run_id, bundle_sha):
    ...
    doc = json.loads(raw)
    if doc.get("schema") not in ("capital-statement-v1", "capital-statement-v2", "capital-statement-v3", "capital-statement-v4") or doc.get("company") != company:
    ...
    if doc.get("schema") in ("capital-statement-v3", "capital-statement-v4"):
