...
test('real accepted 2025-12-31 period has verified totals and exposes original cost components',()=>{
 const d=docs.find(d=>d.period_end==='2025-12-31');assert.ok(d);
 ...
 for(const [block,groups] of Object.entries(m.annual['2025-12-31']))if(['wealth','liquidity'].includes(block))groups.forEach((g,i)=>g.rows.forEach(r=>details[fieldId(`capital:${block}:${i}`,r.label)]=true));
...
test('real annual manifest loads and linked year/detail events work in page runtime',async()=>{
 ...
 click({field:fieldId('capital:wealth:0','经营耗用与费用（折旧、损失前）'),year:'2025-12-31'});
 assert.match(nodes.comparison.innerHTML,/class="aligned-subfield"/);
 click({table:'capital',year:'2025-12-31'});
 assert.doesNotMatch(nodes.comparison.innerHTML,/<th colspan="2">截至 2025-12-31 的期间变动/);
});
