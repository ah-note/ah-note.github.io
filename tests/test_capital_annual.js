...
function fixture(year){
 ...
 put('noncash.leases');
 return {schema:'capital-statement-v1',company:'TEST',currency:'CNY',period_start:`${year}-01-01`,period_end:end(year),records,movements:{},anomalies:[],sources:[],validation:{errors:[],warnings:[]}};
}
...
test('one-year and five-year independent documents preserve all standard fields',()=>{
 for(const years of [[2025],[2021,2022,2023,2024,2025]]){
  ...
  assert.ok(h.includes(`${years[0]-1}-12-31`));
  assert.match(h,/重要异常与一次性事项/);
  assert.doesNotMatch(h,/产品与服务收入/);
  assert.match(view.render(m,{assets:years.map(end),capital:years.map(end)}),/产品与服务收入/);
 }
});
...
test('v2 fact ledger renders economic mapping and keeps source amount visible',()=>{
 const h=view.render(model([fixtureV2(2025)]),{assets:[end(2025)]});
...
test('page hides sub-3-percent discrepancies but retains material anomalies',()=>{
 ...
 assert.match(anomalies,/重大终止确认|需要展示|2025-12-31 · 1 项/);
});
...
test('asset components expand as aligned rows in the parent table',()=>{
 ...
 const state=view.toggleComponent({assets:[end(2021),end(2022)]},id),html=view.render(m,state);
...
test('actual v3 years render only registered Chinese table fields in filing currency',()=>{
 ...
 assert.match(table,/资本表 <small>期末余额 · CNY 亿元/);
 assert.match(table,/资本活动表 <small>期间发生额 · CNY 亿元/);
