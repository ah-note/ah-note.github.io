(function(){
 ...
 const yearButton=(table,year,open,label=String(year))=>'<button type="button" data-table="'+table+'" data-year="'+year+'" aria-expanded="'+(open===year)+'">'+esc(label)+(open===year?' ▾':' ▸')+'</button>';
 ...
 function assetTable(assets,groups,years,open){
  const missing=open===2024,columns=2+years.length+(open?2:0);
  const width=330+years.length*100+(open?420:0);
  let h='<div class="table-wrap"><table class="year-assets" style="width:'+width+'px;min-width:'+width+'px"><caption>资本表 <small>年末余额 · 人民币亿元</small></caption><colgroup>'+col(230)+col(100)+years.map(y=>(y===open?col(320)+col(100):'')+col(100)).join('')+'</colgroup><thead><tr><th rowspan="2">资产／负债项目</th>'+'<th rowspan="2"><span class="balance-label">2023 年末</span><small class="missing">未转录</small></th>'+years.map(y=>(y===open?'<th colspan="2">'+y+' 年变动</th>':'')+'<th rowspan="2"><span class="balance-label">'+y+' 年末</span>'+yearButton('assets',y,open,y+' 年变动')+'</th>').join('')+'</tr><tr>'+(open?'<th>项目</th><th>金额</th>':'')+'</tr></thead><tbody>';
  const total=(label,values,cls='total')=>'<tr class="'+cls+'"><th>'+label+'</th>'+'<td class="missing">未转录</td>'+years.map((y,i)=>(y===open?'<td>净变动</td><td class="amount">'+(missing?'未转录':signed(values[1]-values[0]))+'</td>':'')+amount(values[i])).join('')+'</tr>';
