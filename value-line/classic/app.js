...
document.getElementById('chart').innerHTML=`<svg viewBox="0 0 850 300" role="img" aria-label="迪士尼 2007 至 2016 年年度最高最低股价区间，未补造月度数据"><rect x="30" y="27" width="770" height="230" fill="#fff" stroke="#111"/>${[20,40,60,80,100,120,140].map(v=>`<path d="M30 ${y(v)}H800" stroke="#bbb" stroke-width=".6"/><text x="807" y="${y(v)+4}">${v}</text>`).join('')}${years.map((yr,i)=>`<path d="M${x(i)} 27V257" stroke="#ddd"/><path d="M${x(i)} ${y(hi[i])}V${y(lo[i])}M${x(i)-6} ${y(hi[i])}H${x(i)+6}M${x(i)-6} ${y(lo[i])}H${x(i)+6}" stroke="#111" stroke-width="2"/><text text-anchor="middle" x="${x(i)}" y="${y(hi[i])-7}">${hi[i]}</text><text text-anchor="middle" x="${x(i)}" y="${y(lo[i])+15}">${lo[i]}</text><text text-anchor="middle" x="${x(i)}" y="277">${yr}</text>`).join('')}<text x="34" y="16">竖线：年度最高 / 最低价 · 未复绘原版月线与相对强弱线</text></svg>`;
